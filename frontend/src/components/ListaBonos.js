...
const ListadoNotas = () => {
    ...
    const handleDelete = (id, numero) => {
        if (window.confirm("Are you sure to delete this note?") === true) {
            axios.delete(`https://localhost:7126/Bonos/${id}`)
            .then((result)=>{
    ...
    const handleEdit = (id) => { //para editar notas
        handleShowEdit(true);
        axios.get(`https://localhost:7126/Notes/${id}`)
        .then((result) => {
                                ...
                                return (
                                        <tr key={index}>
                                            ...
                                            <td colSpan={2}>                                                    
                                                ...
                                                <button className='btn btn-danger' onClick={() => handleDelete(item.id, item.numero)}>Delete</button> &nbsp;
                                            </td>
                                        </tr>
                                    )
                                })
